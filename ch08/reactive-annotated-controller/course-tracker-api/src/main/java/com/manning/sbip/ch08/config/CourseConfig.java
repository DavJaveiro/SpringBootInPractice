...
public class CourseConfig {
	...
	public CommandLineRunner init(CourseRepository courseRepository) {
		return args -> {
			...
			Course course1 = Course.builder().author("Mastering Spring Boot").category("Spring").rating(4)
					.description("Mastering Spring Boot").build();
			Course course2 = Course.builder().author("Mastering Python").category("Python").rating(5)
					.description("Mastering Python").build();
			Course course3 = Course.builder().author("Mastering Go").category("Go").rating(3).description("Mastering Go")
					.build();
